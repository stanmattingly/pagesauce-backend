document.addEventListener('DOMContentLoaded', () => {
    ...
    fetch(`http://localhost:8000/auth/smart-add-tokens/${smartAddToken}/`, {
        method: "get",
        headers: {
        ...
        if (response.ok) {
                ...
                document.addEventListener('smartclick', event => {
                    ...
                    if (!isHTML(element.innerHTML) && element.innerHTML) {
                        if (!element.dataset.clicked) {
                            ...
                            fetch("http://localhost:8000/api/app/components/", {
                                method: "post",
                                headers: {
                                    'Authorization': `Token ${apiKey}`,
                ...
                if (!clusterId) {
                    fetch(`http://localhost:8000/api/app/content-clusters/`, {
                        ...
                        xpathComponents.forEach(xpathComponent => {
                            ...
                            fetch(`http://localhost:8000/api/app/components/get_smart_content/?id=${encodeURIComponent(xpathComponent.xpath)}&smart_user_id=${localStorage.getItem('smart-user-id')}&cluster_uuid=${sessionStorage.getItem('smart-cluster-id')}`, {
                                method: 'get',
                                headers: {
                                    'Authorization': `Token ${apiKey}`
                                }
                    ...
                    xpathComponents.forEach(xpathComponent => {
                        const element = xpathComponent.element;
                        fetch(`http://localhost:8000/api/app/components/get_smart_content/?id=${encodeURIComponent(xpathComponent.xpath)}&smart_user_id=${localStorage.getItem('smart-user-id')}&cluster_uuid=${clusterId}`, {
                            method: 'get',
                            headers: {
                                'Authorization': `Token ${apiKey}`
                            }
            ...
            fetch('http://localhost:8000/api/app/components/get_xpath_components/', {
                method: 'get',
                headers: {
                    'Authorization': `Token ${apiKey}`
                }
